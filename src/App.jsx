    ...
    return (
      ...
      >
        <div className="animate-float">
          <SacredLotus size={140} />
        </div>

        <h1 className="font-display text-4xl text-[#C9A962] mt-6 tracking-wide">
          GitaVerse
        </h1>
        ...
        <SacredDivider width={180} className="my-8 opacity-60" />

        <p className="text-[#F5EFE0]/60 text-center text-sm leading-relaxed max-w-xs mb-10">
          Where ancient wisdom meets modern dilemmas. A journey of reflection with family and friends.
        </p>

        <Button onClick={() => setScreen("setup")} className="max-w-xs">
          Begin Journey
        </Button>
        ...
        <p className="text-[#F5EFE0]/30 text-xs mt-10">3–8 Seekers</p>
      </div>
    );
  }
        ...
        {history.length === 0 ? (
          ...
          <div className="space-y-4">
            {[...history].reverse().map((g, i) => (
              <div
                key={i}
                className="rounded-2xl p-5 bg-[#2D1F1A]/50 border border-[#C9A962]/10"
              >
    ...
    return (
      ...
      >
        ...
        {names.length < 8 && (
          <button
            onClick={add}
            className="w-full py-3.5 rounded-xl border border-dashed border-[#C9A962]/30 text-[#C9A962]/60 hover:border-[#C9A962]/50 hover:text-[#C9A962] transition-colors mb-6"
          >
            + Add Seeker
          </button>
        )}
